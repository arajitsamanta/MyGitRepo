package com.arajit.crack.code.ch02.linkedlist;
...
public class Node<T> {
	 ...
	 * @param head
	 */
	public Node<T> appendList(Node<T> thisList,Node<T> that){
		if(thisList==null || that==null)
			return null;
		
		//Temporary runner for this list to traverse
		Node<T> runner;
		runner=thisList;
		
		//Iterate the list until last node
		while(runner.next!=null){
			runner=runner.next;
		}
		
		//Add "that" list to the end of "this" list
		runner.next=that;
		
		//return "this" list after addition
		return thisList;
	}
	...
	public Node<T> rotateUptoKth(Node<T> head,int k){
		
		//Get linked list size
		int size=head.length(head);
		
		// Return null if linked list size is less than the kth element(Where rotate begins) 
		if(head==null || size<k)
			return null;
		
		//Reference to keep head-1 node
		Node<T> headPrev=null;
		
		//Reference to new head for this method to return
		Node<T> newhead=null;
		
		//Initialize temp head to keep head reference
		...
		for(int i=1;i<=size;i++){
			...
			if(i==k){
				headPrev.next=null;
				//reverse list up to k'th position, then append tail to it
				newhead=appendList(reverse(tempHead),head);
				break;
			}
		}		
		return newhead;
